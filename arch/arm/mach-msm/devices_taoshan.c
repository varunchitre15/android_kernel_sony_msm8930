...
static struct persistent_ram_descriptor pram_descs[] = {
	...
	{
		.name = "ram_console",
		.size = TAOSHAN_RAM_CONSOLE_SIZE,
	},
#endif
};

static struct persistent_ram taoshan_persistent_ram = {
	.size = TAOSHAN_PERSISTENT_RAM_SIZE,
...
{
	struct persistent_ram *pram = &taoshan_persistent_ram;
	struct membank* bank = &meminfo.bank[0];

	pram->start = bank->start + bank->size - TAOSHAN_PERSISTENT_RAM_SIZE;
