...
int BMA250_iIsFaceUp(void)
{
    int iIsUp, iIsDown;

    
    iIsUp = (int)atomic_read(&g_aIsFaceUp);
    iIsDown = (int)atomic_read(&g_aIsFaceDwon);

    if(iIsUp ==1 && iIsDown == 0)
        return 1;
    else
        return 0;
}
EXPORT_SYMBOL(BMA250_iIsFaceUp); 


int BMA250_iIsFaceDown(void)
{
    int iIsUp, iIsDown;
...
static void bma250_work_func(struct work_struct *work)
{
    ...
    else if( (acc.z > -270) && (acc.z < -240))
    {
        atomic_set(&g_aIsFaceUp, (unsigned int)0);
		...
		struct device_attribute *attr, char *buf)
{
	...
	comres = bma250_smbus_read_byte(bma250->bma250_client,
			BMA250_EN_SOFT_RESET__REG, &data);
...
#else
static DEVICE_ATTR(softreset, S_IRUGO | S_IWUSR|S_IWGRP|S_IWOTH,
		bma250_softreset_show, bma250_softreset_store);
#endif
...
#if 0
    iGpioData = gpio_get_value(BMA_INT1_GPIO);
    PrintTip("[%s] iGpioData(gpio-%d)=%d..\n", __FUNCTION__, BMA_INT1_GPIO, iGpioData);
    Printhh("[%s] iGpioData(gpio-%d)=%d..\n", __FUNCTION__, BMA_INT1_GPIO, iGpioData);

    iGpioData = gpio_get_value(BMA_INT2_GPIO);
...
static int s_iEnDisPower(bool bEn)
{
    ...
    if(bEn == true)     //turn on power
    {
        ...
        if (rc) {
            PrintTip("[%s] enable l11 failed, rc=%d\n", __FUNCTION__, rc);
    ...
    else
    {
        ...
        if (rc) {
            PrintTip("[%s] disable l11 failed, rc=%d\n", __FUNCTION__, rc);
            pr_err("enable l11 failed, rc=%d\n", rc);
            regulator_put(reg_l11);
            return -ENODEV;
        }
...
*/
    ...
    rc = gpio_request(BMA_INT1_GPIO, "bma250_int1");
    if (rc < 0)
        printk(KERN_ERR"fail to request gpio for comp_int! error=%d\n", rc);
    rc = gpio_direction_input(BMA_INT1_GPIO);
    if (rc <0)
    ...
    if (gpio_tlmm_config(GPIO_CFG(BMA_INT2_GPIO, 0, GPIO_CFG_INPUT,GPIO_CFG_NO_PULL,GPIO_CFG_2MA),GPIO_CFG_ENABLE)) {
        printk(KERN_ERR "%s: Err: Config GPIO-%d \n",	__func__, BMA_INT2_GPIO);
    }

    rc = gpio_request(BMA_INT2_GPIO, "bma250_int2");
    if (rc < 0)
        printk(KERN_ERR"fail to request gpio for comp_int! error=%d\n", rc);
    rc = gpio_direction_input(BMA_INT2_GPIO);
    if (rc <0)
        printk(KERN_ERR "%s: fail to set input = %d\n",__func__,rc );
...
#ifdef BMA250_ENABLE_INT2
    Printhh("[%s] call bma250_set_int2_pad_sel()\n", __FUNCTION__);
	...
	if (err)
    	...
    	err = request_irq(MSM_GPIO_TO_INT(BMA_INT2_GPIO), bma250_irq_handler, IRQF_TRIGGER_RISING,
			"bma250", data);
	if (err)
		printk(KERN_ERR "could not request irq2\n");
